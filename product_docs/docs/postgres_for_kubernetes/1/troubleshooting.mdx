...
```shell
  Image Name:    docker.enterprisedb.com/k8s_enterprise/postgresql:18.0-system-trixie
```
...
EDB Postgres for Kubernetes integrates with [pprof](https://github.com/google/pprof) to
...
-   **Postgres cluster level** – enable by adding the
    `alpha.k8s.enterprisedb.io/enableInstancePprof` annotation to a `Cluster` resource
    (described below).

When the `alpha.k8s.enterprisedb.io/enableInstancePprof` annotation is set to `"true"`,
...
metadata:
  name: cluster-example
  annotations:
    alpha.k8s.enterprisedb.io/enableInstancePprof: "true"
spec:
  instances: 3
  # ...
```
...
First, verify that the cluster has the `alpha.k8s.enterprisedb.io/enableInstancePprof:
"true"` annotation set.
