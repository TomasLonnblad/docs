...
each EDB Postgres for Kubernetes cluster. This method provides you with full control over the
...
!!! important "Important Configuration Details"
    ...
    -   `spec.selector.matchLabels`: You must use the `k8s.enterprisedb.io/cluster: <cluster-name>`
...
```yaml
kubectl -n postgresql-operator-system apply -f - <<EOF
---
apiVersion: monitoring.coreos.com/v1
kind: PodMonitor
metadata:
...
!!! Note
    ...
    with the operator installed in the `postgresql-operator-system` namespace.
...
```sh
kubectl -n postgresql-operator-system port-forward pod/<CONTROLLER-MANAGER-POD> 8080:8080
```
